...
class WarehouseOrder(models.Model):
    user = models.ForeignKey(
        to=User, on_delete=models.DO_NOTHING, related_name="WarehouseOrder"
    )
...
class WarehouseOrderItem(models.Model):
    warehouseorder = models.ForeignKey(
        to=WarehouseOrder,
    ...
    product = models.ForeignKey(
        to=Product,
        on_delete=models.PROTECT,
        related_name="warehouseorderitem",
        verbose_name="Товар",
    )
    purchase_price = models.DecimalField(
        max_digits=7, decimal_places=2, verbose_name="Цена покупки"
    )
